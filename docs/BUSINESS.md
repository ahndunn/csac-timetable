...
| :--- | :--- | :--- | :--- |
| **Admin** | Full system governance: Manage users, promote/demote roles, quorum demotion, event creation, trigger scheduler, view full history. Default authority across all shows and numbers. | User onboarding, quorum ballot reviews (`/admin/approve`), system audit inspection, event lifecycle (`/admin/events`), all Studio views. | **None**: All action controls rendered. |
| **Moderator** | Event operations & scheduling (Superset of DM): Create/close voting events, define time slots, trigger sprint scheduler. Default authority across all shows and numbers. | Event creation (`/admin/events`), schedule finalization, voting monitoring, all DM/PM/QC/Member views. | **Stripped**: User demotion quorum approval (`/admin/approve`). |
| **Delivery Manager (DM)** | Show Lineup & Fleet Director (Superset of PM): Attached via `(user, show)` key. Oversees all music numbers within the designated show, triggers sprint scheduler, manages instrument allocations. | **Trigger Sprint Scheduler**, inspect full audit history (compute & free-time registration history), manage all numbers in Bento/Kanban/Table views across the show. | **Stripped**: User onboarding & role demotion (`/admin/users`, `/admin/approve`). |
| **Performance Manager (PM)** | Music Number Leader (Superset of QC): Attached via `(user, show, music_number)` key. Leads designated Music Numbers, assigns performers & gear, manages Study/Create/Review tasks for their assigned song(s). | Manage assigned numbers, performer lineup configuration, inspect compute & registration history, submit task reviews for assigned numbers. | **Stripped**: Show-wide scheduler trigger, admin pages (`/admin/*`), and editing non-assigned songs. |
| **Quality Reviewer (QC)** | Quality Auditor (Superset of Member): Attached via `(user, show, music_number)` key. Reviews designated music number tasks and submits milestone audit verdicts. | Member views + QC Audit Workstation tab in Music Numbers for assigned numbers. | **Stripped**: Number creation, performer assignments, scheduler trigger, user administration. Enabled ONLY for designated QC number reviews. |
| **Member** | Performer baseline: Attached to songs via `(user, show, music_number)` performer lineup. Registers 15-minute practice sprint free-time grid, views assigned numbers & practice schedule calendar. | Show Studio overview, Music Numbers view, Practice Sprint free-time registration grid & auto-scheduled calendar, Instrument Fleet. | **Stripped**: All action buttons for create/edit/delete numbers, instruments, tasks, scheduler trigger, or admin pages. Non-actionable elements only. |

### 2.2 Key-Based Scoped Role Resolution Model
* **BR-ROL-01 (Zero Generic Role Pickers)**:
  * Generic role dropdown selectors are strictly prohibited on features. Roles are never chosen as an isolated dropdown state on an action screen.
  * System permissions are dynamically resolved from the compound hierarchy:
    $$\text{EffectiveRole}(u, s, n) = \begin{cases} \text{Admin / Moderator} & \text{if } u \text{ has global system role} \\ \text{DM} & \text{if } (u, s) \in \text{ShowDMs} \\ \text{PM} & \text{if } (u, s, n) \in \text{NumberPMs} \\ \text{QC} & \text{if } (u, s, n) \in \text{NumberQCs} \\ \text{Performer / Member} & \text{otherwise} \end{cases}$$
* **BR-ROL-02 (Multi-Role Participation)**:
  * A single user can simultaneously hold distinct roles across different songs (e.g., PM on Song 1, QC on Song 2, Bass Performer on Song 3), while a Show DM holds show-wide leadership over all numbers in Show $s$.

### 2.3 Cross-Screen Synchronization & Deep Linking Rules
* **BR-NAV-01 (Overview $\rightarrow$ Numbers Stage Filter)**: Clicking any stage metric pill on the Overview page navigates to `/studio/shows/[id]/numbers?stage=[stage_name]` with that filter active.
* **BR-NAV-02 (Numbers $\rightarrow$ Sprints Song Timetable)**: Each song card in the Numbers page provides a direct link to `/studio/shows/[id]/sprints?song=[song_title]` to view all scheduled rehearsals for that song.
